...
const students = [
    {
        ...
        program: client_1.Program.WEBDEV,
        batchYear: 2024,
    },
    {
        username: 'student2',
    ...
    },
    {
        ...
        program: client_1.Program.DATA_ANALYST,
        batchYear: 2025,
    },
    {
        username: 'student4',
        ...
        program: client_1.Program.MARKETING,
        batchYear: 2025,
    },
];
const instructors = [
...
const categories = [
    { name: 'Frontend', programs: [client_1.Program.WEBDEV] },
    { name: 'Backend', programs: [client_1.Program.WEBDEV] },
    { name: 'Data Visualization', programs: [client_1.Program.DATA_ANALYST] },
    { name: 'Machine Learning', programs: [client_1.Program.DATA_ANALYST] },
    { name: 'Digital Marketing', programs: [client_1.Program.MARKETING] },
];
...
async function insertData() {
    ...
    const seededStudents = await Promise.all(students.map(async (student) => {
        ...
        return prisma.student.create({
            data: {
                ...
                membershipStatus: client_1.MembershipStatus.APPROVED,
            },
        });
    }));
    const seededInstructors = await Promise.all(instructors.map(async (instructor) => {
        const user = await prisma.user.create({
            data: {
    ...
    console.log(`Created ${seededStudents.length} students and ${seededInstructors.length} instructors.`);
    ...
    if (!mlCategory)
        throw new Error('mlCategory not found during seeding!');
    ...
    const course1 = await prisma.course.create({
        data: {
            ...
            description: 'A comprehensive course on building robust backend APIs with NestJS and Prisma.',
            allowedPrograms: [client_1.Program.WEBDEV],
    ...
    const course2 = await prisma.course.create({
        data: {
            ...
            description: 'An introduction to Python programming for data scientists and analysts.',
            allowedPrograms: [client_1.Program.DATA_ANALYST],
    ...
    const course3 = await prisma.course.create({
        data: {
            ...
            allowedPrograms: [client_1.Program.MARKETING],
            allowedBatchYears: [2025],
            isMemberOnly: false,
            categories: {
    ...
    console.log(`Created courses: ${course1.title}, ${course2.title}, and ${course3.title}.`);
    ...
    const module1_1 = await prisma.module.create({
        data: {
            ...
            moduleType: client_1.ModuleType.LECTURE,
    ...
    const module1_2 = await prisma.module.create({
        data: {
            ...
            moduleType: client_1.ModuleType.LECTURE,
            description: 'Learn how to use Prisma for database access and migrations.',
            sectionId: section1.id,
        },
    });
    const module1_3 = await prisma.module.create({
        data: {
            ...
            moduleType: client_1.ModuleType.ASSIGNMENT,
    ...
    console.log(`Created submission template and fields for assignment: ${module1_3.title}.`);
    const enrollment1 = await prisma.enrollment.create({
        data: {
            studentId: seededStudents[0].id,
            instructorId: seededInstructors[0].id,
            courseId: course1.id,
            status: client_1.CourseStatus.ENROLLED,
            moduleProgresses: {
                create: [
    ...
    const submission2 = await prisma.submission.create({
        data: {
            ...
            scoreAchieved: 95.5,
            scoreTotal: 100,
            feedback: 'Excellent work, Alice! The project is well-designed and functional.',
        },
    });
